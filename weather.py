...
def call_llm(system_prompt: str, user_prompt: str, model: str = "gpt-3.5-turbo",
...
def create_flask_app():
    ...
    def generate_report():
        ...
        # Get the provider
        provider = WeatherProviderFactory.create_provider(provider_name, cache)
        if not provider:
            return render_template('error.html',
        ...
        try:
            if report_type == 'current':
                # Get current weather
                weather_data = provider.get_current_weather(city, units)
                if not weather_data:
                    return render_template('error.html',
                ...
                weather_data = provider.get_historical_weather(city, historical_date, units)
                if not weather_data:
                    return render_template('error.html',
                ...
                # Get forecast
                forecast_data = provider.get_forecast(city, forecast_days, units)
                ...
                for location in comparison_locations:
                    weather = provider.get_current_weather(location, units)
                    if weather:
                        # Add geographic context if enabled
                        if DEFAULT_CONFIG.get("geolocation_enabled", True):
                ...
                if not weather_data_list:
                    return render_template('error.html',
                                         message="Could not get weather data for any of the provided locations")
    ...
    @app.route('/templates')
    def get_templates():
        """Return a dictionary of simple templates for testing without actual template files"""
        templates = {
            "index.html": """
                <!DOCTYPE html>
                <html>
